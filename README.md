...
- Bring up Jenkins with all required tooling via `docker compose -f infra/jenkins/docker-compose.yml up -d --build`. The entrypoint automatically matches the container `docker` group to the socket you mount (`/var/run/docker.sock` by default). For Colima, use `docker --context colima compose` if your default context is elsewhere.
